...
<div class="content-wrapper pb-0">
    ...
    $options = ['NO', 'ID', 'LEVEL', 'STATUS', 'DATE', 'SYSTEM', 'COMPONENT', 'SUPPORT TYPE', 'CUSTOMER', 'COMPANY', 'PROJECT', 'ACTIONS'];
    $optionLabels = [
        'NO' => 'No',
        'ID' => 'ID',
        'LEVEL' => 'Level',
                ...
                <table id="fresh-table" class="table" data-show-export="true">
                    ...
                    <thead>
                        ...
                        foreach ($selectedOptions as $option) {
                            ...
                            if ($option == "LEVEL") {
                                echo "<th class='level-column' data-field=\"{$option}\">{$label}</th>";
                            } else {
                        ...
                        <tr>
                            ...
                            <?php if (in_array('ID', $selectedOptions)) {
                                if (isset($ticket->strTicketUniqueID) && $ticket->strTicketUniqueID != '') {
                                    echo '<td>' . $ticket->strTicketUniqueID . '</td>';
                                } else {
                                    echo '<td>' . str_pad($ticket->iTicketId, 4, '0', STR_PAD_LEFT) . '</td>';
                                }
                            } ?>
                            <?php if (in_array('LEVEL', $selectedOptions)) {
                                // $Level = $ticket->closelevel ? $ticket->closelevel : $ticket->openLevel;
                                // echo '<td>Level ' . ($Level ?? 2) . '</td>';
                                echo '<td>Level ' . ($ticket->closelevel ? $ticket->closelevel : $ticket->openLevel) . '</td>';
                            } ?>
...
<?php if (in_array('46', \Session::get('menuList'))) { ?>
    <script>
        ...
        $(document).ready(function() {
            $('#fresh-table th[data-field="LEVEL"]').css({
                'width': '7%',
            });
